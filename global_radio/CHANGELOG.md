...
- **CI/CD release pipelines**, both building on GitHub-hosted runners (no local build required):
  ...
  - Both apply the full set of production `--dart-define` flags, including `DEMO_AUDIO=false`, and fail fast with an explicit message listing exactly which secrets are missing rather than a deep, confusing build error.
